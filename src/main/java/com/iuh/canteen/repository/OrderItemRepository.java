...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT COUNT(DISTINCT oi.orders.id) FROM OrderItem oi WHERE oi.stall.id = :stallId")
    ...
    List<Orders> findTop5OrdersByStallId(@Param("stallId") Long stallId, Pageable pageable);
    ...
    @Query("SELECT MONTH(o.createdAt) AS month, SUM(oi.price * oi.quantity) AS totalSales " +
            ...
            "ORDER BY month")
    List<Object[]> findSalesGroupedByMonthAndStall(@Param("stallId") Long stallId);

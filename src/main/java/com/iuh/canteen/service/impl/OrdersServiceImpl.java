...
public class OrdersServiceImpl implements OrdersService {
     ...
     * @param dishesList          Danh sách Món ăn
     ...
     */
    @Override
    public Orders addOrder(String username, List<DishesDTO> dishesList, String paymentMethod,
                           Long selectedPromotionId,
                           String address) {
        ...
        try {
            ...
            for (DishesDTO dishesDTO : dishesList) {
                ...
                Dishes dishes = dishesRepository.findById(dishesDTO.getId())
                                                .orElseThrow();
                dishesService.subtractQuantity(dishesDTO.getId(), dishesDTO.getQuantity());
    ...
    public List<TransactionData> findMonthlyTransactions() {
        ...
        for (Object[] item : tempGroup) {
            ...
            TransactionData temp = result.stream()
                                         ...
                                         .findFirst()
                                         .orElseThrow();
            if (temp != null && order != null && order.getTotalPrice() != null) {
                ...
                if (totalSales.compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    System.err.println("Error: totalSales is non-positive or invalid");
                }
            }
        }
        result.forEach(
    ...
    public Orders findById(Long orderId) {
        ...
        Orders result;
        try {
            result = ordersRepository.findById(orderId)
                                     .orElseThrow();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public Boolean updateStatusByOrderItemId(Long itemId) {
        ...
        try {
            ...
            if (orders == null || orders.getOrderItems() == null || orders.getOrderItems()
                                                                          .isEmpty()) {
                throw new IllegalStateException("Order or Order Items not found.");
            }
            ...
            for (OrderItem orderItem : orders.getOrderItems()) {
                ...
                if (!status.equals(OrderStatus.COMPLETED)) isAllCompleted = false;
                if (status.equals(OrderStatus.PROCESSING)) isProcessing = true;
                if (status.equals(OrderStatus.CANCELLED)) isCanceled = true;
                if (status.equals(OrderStatus.PAID)) hasPaid = true;
                if (status.equals(OrderStatus.PENDING)) hasPending = true;
                if (status.equals(OrderStatus.NEW)) isNew = true;
                if (!(status.equals(OrderStatus.WAITING) || status.equals(OrderStatus.SHIPPING))) {
            ...
            if (isAllCompleted) {
                ...
                orders.setStatus(orders.getAddress() == null || orders.getAddress()
                                                                      .isEmpty()
                        ? OrderStatus.WAITING : OrderStatus.SHIPPING);
                ...
                orders.setStatus(OrderStatus.PENDING); // Default to PENDING if no other status applies
            }
    ...
    /**
     * getProcessingOrdersByUsername Lấy danh sách Đơn hàng đang trong quá trình thực hiện cho Thực khách
     *
     * @param username Tên Tài khoản
     * @return List OrdersDTO
     */
    ...
    /**
     * getWaitingOrdersByUsername Lấy danh sách Đơn hàng đang trong quá trình chờ Thực khách đến nhận
     *
     * @param username Tên Tài khoản
     * @return List OrdersDTO
     */
    ...
    /**
     * addOrderToResult Chuyển đối tượng Đơn hàng sang dữ liệu và thêm vào danh sách
     *
             ...
             .forEach(orderItem ->
                     {
                         ...
                         orderItemDTO.setDishesDTO(modelMapper.map(orderItem.getDishes(), DishesDTO.class));
                         StallDTO stallDTO = modelMapper.map(orderItem.getStall(), StallDTO.class);
                         orderItemDTO.setStallDTO(stallDTO);
                         orderItemDTOS.add(orderItemDTO);
                     }
             );
        orderDTO.setOrderItemDTOS(orderItemDTOS);
        orderDTO.setPaymentDTO(modelMapper.map(order.getPayment(), PaymentDTO.class));
        result.add(orderDTO);
    }
}
